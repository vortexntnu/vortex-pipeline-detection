...
int LinedetectorPipe::detectSingleLine(const mat &points, const mat &values,
...
vector<Line> LinedetectorPipe::operator()(const cv::Mat &img,
  ...
  for (int i = 0; i < maxLines; ++i) {
    ...
    if (returnCode) {
      ...
      if (returnCode) {
        cout << "RANSAC failed to find line number " << i + 1 << " again"
             << endl;
        continue;
      }
    ...
    mat newValues;
    for (size_t j = 0; j < points.n_rows; ++j) {
      if (find(randsac.rempointids.begin(), randsac.rempointids.end(), j) ==
          randsac.rempointids.end()) {
