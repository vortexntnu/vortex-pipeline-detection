...
void PipelineFilteringNode::check_and_subscribe_to_image_topic() {
    ...
    if (image_topic_ != image_topic) {
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            image_topic, qos_profile, std::bind(&PipelineFilteringNode::image_callback, this, _1));
...
void PipelineFilteringNode::create_image_publisher() {
    ...
    if (image_pub_topic_ != pub_topic) {
        image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(pub_topic, qos_profile);
        image_pub_topic_ = pub_topic;
        RCLCPP_INFO(this->get_logger(), "Created publisher for filtered image: %s", pub_topic.c_str());
    }
}
...
void PipelineFilteringNode::on_parameter_event(const rcl_interfaces::msg::ParameterEvent & event) {
    ...
    for (const auto& changed_parameter : event.changed_parameters) {
        if (changed_parameter.name.find("image_sub_topic") == 0) check_and_subscribe_to_image_topic();
        if (changed_parameter.name.find("filter_params") == 0) set_filter_params();
        if (changed_parameter.name.find("image_pub_topic") == 0) create_image_publisher();
    } 
}
