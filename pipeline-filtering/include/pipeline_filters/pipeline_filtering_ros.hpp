...
private:
   ...
   * subscription is cancelled and a new one is bound to the callback function.
   *
   */
   ...
   * Checks for parameter changes that matches the nodes' namespace and invokes
   * the relevant initializer functions to update member variables.
   *
   ...
   * operational behavior dynamically, allowing adjustments without altering the
   * code. The `param_handler_` is responsible for registering callbacks that
   * are triggered on parameter changes, providing a centralized management
   * system within the node for such events.
   */
   ...
   *
   * Represents a token or reference to the specific callback registration made
   * with the parameter event handler (`param_handler_`). This handle allows for
   * management of the lifecycle of the callback, such as removing the callback
   * if it's no longer needed. It ensures that the node can respond to parameter
   * changes with the registered callback in an efficient and controlled manner.
   */
