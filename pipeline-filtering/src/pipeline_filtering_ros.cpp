...
PipelineFilteringNode::PipelineFilteringNode(const rclcpp::NodeOptions& options)
    ...
    optimal_threshold_publisher_ =
        this->create_publisher<std_msgs::msg::Int32>("/optimal_threshold", 10);
    auto_gamma_publisher_ =
        this->create_publisher<std_msgs::msg::Float32>("/auto_gamma", 10.0);
}
...
void PipelineFilteringNode::image_callback(
    ...
    // Calculate optimal threshold and auto gamma as an example (you'll need to
    // replace these with your actual calculations)
    int optimalThreshold = 123;  // calculateOptimalThreshold(input_image); //
                                 // Replace with actual calculation logic
    float autoGamma = 1.23;  // calculateAutoGamma(input_image);             //
                             // Replace with actual calculation logic

    int channels = input_image.channels();
    RCLCPP_INFO_ONCE(rclcpp::get_logger("image_info"), "Number of channels: %d",
                     channels);
    ...
    cv_bridge::CvImage(msg->header, "mono8", filtered_image)
        .toImageMsg(*message);
