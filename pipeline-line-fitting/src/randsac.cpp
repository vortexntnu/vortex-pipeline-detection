...
double value(const mat &inliersVal) { return inliersVal.size(); }

// LinearRegressor
LinearRegressor::LinearRegressor() { params = {}; }

LinearRegressor::~LinearRegressor() { params.clear(); }
...
void LinearRegressor::fit(const mat &X, const mat &y) {
  ...
  if (X.min() == X.max() && X.size() >= 5) { // Threshold for condition number
    ...
    // For example, you can set params to a default value or use a
...
mat LinearRegressor::predict(const mat &X) {
  if (vertical) {
    return ones(X.n_rows) *
           params.at(0); // the vertical line will return x-predections, instead
                         // of y-predictions
  }
...
mat RANDSAC::predict(const mat &X) { return bestFit.predict(X); }

void RANDSAC::fit(const mat &X, const mat &y, const mat &values,
  ...
  for (int i = 0; i < k; i++) {
    ...
    if (maybe_model.vertical && false) {
      ...
      x_pred = maybe_model.predict(
          y.rows(conv_to<uvec>::from(vector<int>(ids.begin() + n, ids.end()))));
      thresholded = loss(X.rows(conv_to<uvec>::from(
                             vector<int>(ids.begin() + n, ids.end()))),
                         x_pred) < t;
    } else {
      others++;
      y_pred = maybe_model.predict(
          X.rows(conv_to<uvec>::from(vector<int>(ids.begin() + n, ids.end()))));
      thresholded = loss(y.rows(conv_to<uvec>::from(
                             vector<int>(ids.begin() + n, ids.end()))),
                         y_pred) < t;
    }
    ...
    // if (maybe_model.vertical){cout << "inlier_ids: " << inlier_ids.size() <<
    // " " << maybe_model.params.at(0) << endl;}
    ...
    if (inlier_ids.size() > 10 && legalAlpha(maybe_model.params[1], lines)) {
      ...
      if (thisValue > bestScore) {
        ...
        mat rem_y_pred = better_model.predict(X.rows(
            conv_to<uvec>::from(vector<int>(ids.begin() + n, ids.end()))));
        mat rem_x_pred;
        rem_thresholded = loss(y.rows(conv_to<uvec>::from(
                                   vector<int>(ids.begin() + n, ids.end()))),
                               rem_y_pred) < remT;

        if (better_model.vertical) {
          vertScore++;
          rem_x_pred = better_model.predict(y.rows(
              conv_to<uvec>::from(vector<int>(ids.begin() + n, ids.end()))));
          rem_thresholded = loss(X.rows(conv_to<uvec>::from(
                                     vector<int>(ids.begin() + n, ids.end()))),
                                 rem_x_pred) < remT;
        } else {
          otherScore++;
  ...
  cout << "Verticals with score: " << vertScore << " Others with score: " <<
  otherScore << endl;*/
}
